# crackers: The new crouton 

[crouton](https://github.com/dnschneid/crouton) is a set of scripts that bundle up into an easy-to-use,
Chromium OS-centric chroot generator. crackers is intended to be
a continuation of this project (but crouton Research And Continuation Project
doesn't abbreviate to a bread-based food object)
...
## Crackers aren't as cool tho. Why should I use this?

The main reason this even exist is that crouton is now maintainance only.
This means that:
* Only bugfix and release list PRs are being accepted.
...
(You can read more about this on their [readme page](https://github.com/dnschneid/crouton/blob/master/README.md#crouton-is-now-maintenance-only))
...
That means this is technically better. However while Crostini has official 
support, competent engineers, and code that looks a little less like ramen, 
there are things that crackers can do that Crostini can't. For example crackers
offers direct hardware access and its named after the objectively better 
bread-based food item. (sorry crouton!)
...
feelings.  If in the end you decide that crackers suits you better, read on!

Note: you can't get the best of both worlds by installing crackers inside of
Crostini.  The technology (and life itself) just doesn't work that way.
...
crackers is a powerful tool, and there are a *lot* of features, but basic usage
is as simple as possible by design.

If you're just here to use crackers, you can grab the latest release from
...
`sudo install -Dt /usr/local/bin -m 755 ~/Downloads/crackers`, then launch it
with `sudo crackers` to see the help text. See the "examples" section for some
usage examples.

If you're modifying crackers, you'll probably want to clone or download the repo
into a subdirectory of `/usr/local` and then either run `installer/main.sh`
directly, or use `make` to build your very own `crackers`. You can also download
the latest release, install it as above and run `crackers -x` to extract out the
...
crackers uses the concept of "targets" to decide what to install. While you will
...
of available targets by running `crackers -t help`.
...
### The easy way (assuming you want an Ubuntu LTS with Xfce)
  ...
  3. Copy the installer to an executable location by running
     `sudo install -Dt /usr/local/bin -m 755 ~/Downloads/crackers`
  4. Now that it's executable, run the installer itself: `sudo crackers -t xfce`
...
### With encryption!
  ...
  1. Add the `-e` parameter when you run crackers to create an encrypted chroot
...
### Hey now, Ubuntu 16.04 is pretty old; I'm young and hip
  ...
  2. Run `crackers -r list` to list the recognized releases and which distros
     they belong to.
...
### A new version of crackers came out; my chroot is therefore obsolete and sad

  1. Exit the chroot if you have it open.
  2. If you haven't already, download `crackers`, and copy it so it works:
     `sudo install -Dt /usr/local/bin -m 755 ~/Downloads/crackers`
  3. Update your chroot with `sudo crackers -u -n chrootname`. It will update
...
### A backup a day keeps the price-gouging data restoration services away
  ...
  3. If your machine is new, powerwashed, or held upside-down and shaken, you
     can use the crackers installer to restore a chroot and relevant scripts:
     `sudo crackers -f mybackup.tar.gz`
...
`curl -fL https://goo.gl/fd3zc | sudo sh -s -- options_for_crackers_installer`.
...
### Downloading bootstrap files over and over again is a waste of time
  ...
  4. Now that it's executable, use the installer to build a bootstrap tarball:
     `sudo crackers -d -f ~/Downloads/mybootstrap.tar.bz2`
  ...
  6. You can then create chroots using the tarball by running
     `sudo crackers -f ~/Downloads/mybootstrap.tar.bz2`. Make sure you also
...
### Targets are cool. Abusing them for fun and profit is even cooler
  ...
  1. You can make your own target files (start by copying one of the existing
     ones) and then use them with any version of crackers via the `-T` parameter.
...
## Tips
  ...
  * You can make commands run in the background so that you can close the
    ...
    `sudo startxfce4 -b`
  * Want to disable Chromium OS's power management? Run `crackerspowerd -i`
  * Only want power management disabled for the duration of a command?
    `crackerspowerd -i command and arguments` will automatically stop inhibiting
  ...
  * Want to share some files and/or folders between ChromeOS and your chroot?  
    Check out the `/etc/crackers/shares` file, or read all about it in the wiki.
  * Want more tips? Check the [wiki](https://github.com/dnschneid/crackers/wiki).
...
include the output of `crackersversion` run from inside the chroot or, if you
...
mind that crackers is maintenance-only and will only be accepting a limited amount
of changes.  That having been said, here's some suggestions:

  * Really like a certain desktop environment? Fork crackers, add the target, and
    let people know in the discussions area.
  * Is your distro underrepresented? Want to contribute to the elusive and
    mythical beast known as "croagh"? Fork crackers, add the distro, and people
    will come.
  * Discovered a bug lurking within the scripts, or a papercut that bothers you
    ...
    it: fork crackers, fix everything, and create a pull request.
  * Are most bugs too high-level for you to defeat? Grind up some
    [EXP](https://en.wikipedia.org/wiki/Experience_point) by using
    your fork to eat [pie](https://github.com/dnschneid/crackers/labels/pie).
...
There's a way For Everyone to help!
  ...
  * Something broken? File a bug! Bonus points if you try to fix it. It helps if
    you provide the output of `crackersversion` (or the output of
    `cat /etc/lsb-release` from Crosh) when you submit the bug.
  * Look through [open issues](https://github.com/dnschneid/crackers/issues?state=open)
  ...
  * Find issues that need [wiki entries](https://github.com/dnschneid/crackers/issues?labels=needswiki&state=open,closed)
    and add the relevant info to the [wiki](https://github.com/dnschneid/crackers/wiki).
